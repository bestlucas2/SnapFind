...
queue (Celery/RQ) later is a localized change. File storage is pluggable
(`services/storage.py`): local disk for dev, Supabase Storage for cloud. Full-text
search uses portable `ILIKE`; a Postgres `tsvector` + GIN index is the natural next
step at scale.

---

## Deploy to the cloud (Render + Supabase)

SnapFind runs anywhere that can run a Docker container. The included `Dockerfile`
bundles Tesseract; in the cloud it uses **Supabase Postgres** for data and
**Supabase Storage** for uploaded images (the container disk is ephemeral).

**1. Supabase** — get these from your project:
- Database connection string: *Project Settings → Database → Connection string → URI →* **Session pooler** (IPv4-friendly). Prefix the driver: `postgresql+psycopg://…`.
- `Project Settings → API` → **Project URL** and the **service_role** key.
- Enable **RLS** on all tables (Table Editor → each table → *Enable RLS*). The app
  connects as the `postgres` owner and bypasses RLS, so this only blocks anonymous
  API access — it won't break the app.

**2. Render** — *New → Blueprint*, connect this repo (it reads `render.yaml`), then
set these env vars (the secrets are marked `sync: false`):
...
**3. Tables + demo data.** Tables auto-create on first boot. To seed demo data
*into Supabase Storage*, run once with the cloud env (locally or via Render shell):
```bash
STORAGE_BACKEND=supabase SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... \
DATABASE_URL=... python -m seed.seed --force
```

That's it — share the Render URL and others can register and use it. Locally,
nothing changes: leave `STORAGE_BACKEND` unset and it uses the `uploads/` folder.
